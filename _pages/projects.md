...
Leverage additional unlabelled data to develop a Self-Training approach for 3D segmentation of Electron Microscopy volumes. Built a Residual Symmetric 3D U-Net for model predictions. Used a pre-trained model to generate pseudo-labels for additional unlabeled volumes. Retrained the model using labelled and pseudo-labeled volumes to improve adapted rand index on test volumes. Converted binary foreground probability maps, instance contours and signed distance transform to instance masks via the watershed segmentation algorithm.
